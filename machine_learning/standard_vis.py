...
def get_important_features(ft_set, feature_importance, show_cols = 30, cutoff = 0.5):
    w_lr_sort, ft_sorted, _ = return_feature_importance(ft_set, feature_importance, show_cols = show_cols)
    to_return = [];
    for i in range(len(ft_sorted)):
        if w_lr_sort[i]>cutoff:
            to_return.append(ft_sorted[i])
        else:
            break
    return to_return
...
def plot_test_train(Y_label, Test_Y_True, Test_Y_Predicted, Y_pre, range_set = True, range_x=[-1.5,3.5], range_y=[-1.5,3.5], tag='Train', mod='model', axis_label=' value'):
    from scipy.optimize import curve_fit
    import numpy as np
    import matplotlib.pyplot as plt
    from scipy import stats
    ...
    plt.scatter(Y_label,pre_Y,
                ...
                facecolors='none', 
                edgecolors='b')
    plt.scatter(Test_Y_True,Test_Y_Predicted,
                marker='.',
                s=80, linewidth=2,
                facecolors='none', 
                edgecolors='g')
    plt.legend(['Best Fit','Perfect Fit', 'Training Data', 'Test Data'], loc='lower right')
    plt.xlabel('True '+axis_label)
    plt.ylabel('Predicted '+axis_label)